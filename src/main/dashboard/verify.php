...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['verifationCodeSubmit'])){
    ...
    if($eresult['verificationCode'] == $_POST['verificationCode']){
        if (empty($submittedVerificationCode)) {
            $errors[] = 'Por favor, introduce el código de verificación.';
        }
        else if (strlen($submittedVerificationCode) !== 6) {
            $errors[] = 'El código de verificación tiene una longitud incorrecta.';
        }
        else if (!ctype_digit($submittedVerificationCode)) {
            $errors[] = 'El código de verificación contiene caracteres inválidos.';
        }

        if(strtotime($eresult['vericationCodeDate']) < strtotime('-1 hour')){
            $errors[] = 'El código de verificación ha expirado. Por favor, solicita un nuevo código.';
        }
        if (empty($errors)) {
            if ($eresult['verificationCode'] == $submittedVerificationCode) {
                if (strtotime($eresult['verificationCodeDate']) < strtotime('-1 hour')) {
                    $errors[] = 'El código de verificación ha expirado. Por favor, solicita un nuevo código.';
                } else {
                    try{
                        $statement = $connection->prepare('UPDATE users SET verificationCode = :verificationCode WHERE id = :id');
                        $statement->execute(array(
                            ':status' => "verified",
                            ':id' => $id,
                        ));

                        $estatement = $connection->prepare('UPDATE users SET verificationCode = :verificationCode, vericationCodeDate = :vericationCodeDate, lastUserVerification = :lastUserVerification WHERE userId = :userId');
                        ...
                        $connection->rollBack();
                        error_log("SIGNUP (1): Database error inserting: " . $e->getMessage());
        ...
        try{
            $statement = $connection->prepare('UPDATE usersCode SET verificationCode = :verificationCode, vericationCodeDate = :vericationCodeDate WHERE userId = :userId');
            $statement->execute(array(
                ':verificationCode' => $verificationCode,
                ':userId' => $id,
                ':vericationCodeDate' => date('Y-m-d H:i:s')
            ));
        } catch (PDOException $e) {
            $connection->rollBack();
            error_log("SIGNUP (1): Database error inserting: " . $e->getMessage());
